...
echo "[C] replay finished, logs: $LOG_DIR"

if [[ -n "${PROMPT_REF_MD5:-}" && -f "${PROMPT_FILE:-Data/result/Bdm2/pniCoin/four_machine_correctness/prompt.lmf}" ]]; then
  ACTUAL_PROMPT_MD5=$(md5sum "${PROMPT_FILE:-Data/result/Bdm2/pniCoin/four_machine_correctness/prompt.lmf}" | awk '{print $1}')
  echo "[C] prompt md5 expected=$PROMPT_REF_MD5 actual=$ACTUAL_PROMPT_MD5"
fi

if [[ -n "${DELAY_REF_MD5:-}" && -f "${DELAY_FILE:-Data/result/Bdm2/pniCoin/four_machine_correctness/delay.lmf}" ]]; then
  ACTUAL_DELAY_MD5=$(md5sum "${DELAY_FILE:-Data/result/Bdm2/pniCoin/four_machine_correctness/delay.lmf}" | awk '{print $1}')
