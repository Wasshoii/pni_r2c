...
LOG_DIR="app/experiments/logs/balanced_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
...
cleanup() {
  ...
  pkill -f "bin/app_coin_master --config app/config/experiments/coin_master_balanced.json" >/dev/null 2>&1 || true
}
trap cleanup EXIT INT TERM

make app-coin-master app-acq-r2s-node app-udp-replayer

./bin/app_coin_master --config app/config/experiments/coin_master_balanced.json > "$LOG_DIR/coin_master.log" 2>&1 &
sleep 2
...
./bin/app_udp_raw_replayer \
  ...
  --channel-count 4 --channel-offset 0 --max-segments 120 --inter-packet-us 1 --repeat 1 > "$LOG_DIR/replayer_0.log" 2>&1 &
RPIDS+=("$!")
...
./bin/app_udp_raw_replayer \
  ...
  --channel-count 4 --channel-offset 0 --max-segments 120 --inter-packet-us 1 --repeat 1 > "$LOG_DIR/replayer_1.log" 2>&1 &
RPIDS+=("$!")
...
./bin/app_udp_raw_replayer \
  ...
  --channel-count 4 --channel-offset 0 --max-segments 120 --inter-packet-us 1 --repeat 1 > "$LOG_DIR/replayer_2.log" 2>&1 &
RPIDS+=("$!")
...
echo "Balanced run finished. Logs: $LOG_DIR"
