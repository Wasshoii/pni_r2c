...
- **打包**：host span 可 `memcpy`；device span 由 `CoincidenceClient` 用消费线程专用 non-blocking stream 按槽 `cudaMemcpyAsync` 进 TX。见 [PackedSingle](../core/streaming/PackedSingle.md)。
- **发送**：RoCE 下 `sendSingles` 在 **有序 `next()` 之后的单消费线程** 里 `acquireTxSlot` + 专用 copy stream D2H 进 TX + 当场 `commitTxSlot`。禁止 GPU worker 自己填槽（完成序 ≠ 段序）。InProcess 对 device span 先同步 D2H 再当场 `sendPackedSingles`。握手与 PAUSE 见 [状态机与调试.md](../../app以及实验配置/状态机与调试.md)。
...
BDM50100 + RoCE、不写盘：raw 2 次（host memcpy + H2D）；singles **一次 D2H 直写 TX MR**（消费线程专用 copy stream，已 `cudaHostRegister`），不经整段 pinned host。单槽 ingest 再 unpack 一次。跨槽 ingest 一次组装后 move。发送缓冲与填槽见 [dataplane/rdma/README.md](../dataplane/rdma/README.md#发送端缓冲)。GPUDirect（未实现）见 [GPUDirectRDMA.md](../dataplane/rdma/GPUDirectRDMA.md)。
