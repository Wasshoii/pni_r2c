...
- **打包**：host span 可 `memcpy`；device span 由 `CoincidenceClient` 按 GPU 缓存的 non-blocking stream 按槽 `cudaMemcpyAsync` 进 TX。见 [PackedSingle](../core/streaming/PackedSingle.md)。
- **发送**：RoCE 下 `sendSingles` 在 **有序 `next()` 之后的单消费线程** 里 `acquireTxSlot` + 该 GPU 常驻 copy stream D2H 进 TX + 当场 `commitTxSlot`。禁止 GPU worker 自己填槽（完成序 ≠ 段序）。InProcess 对 device span 先同步 D2H 再当场 `sendPackedSingles`。握手与 PAUSE 见 [状态机与调试.md](../../app以及实验配置/状态机与调试.md)。
