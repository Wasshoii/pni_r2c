...
namespace openpni::distributed::acquisition
{
    ...
    private:
        ...
        static AcquisitionTask BuildEffectiveGlobalTask(const AcquisitionTask &task)
        {
            ...
            // Explicit per-channel config has the highest priority.
    ...
    inline AcquisitionMaster::AcquisitionMaster()
    ...
    inline void AcquisitionMaster::Initialize(const AcquisitionTask &global_task)
    {
        global_task_ = global_task;
    }

    inline void AcquisitionMaster::StartServer(const std::string &server_address)
    {
        if (server_)
        {
    ...
    inline void AcquisitionMaster::Wait()
    {
        if (server_)
        {
            server_->Wait();
    ...
    inline void AcquisitionMaster::StopServer()
    {
        if (server_)
        {
            server_->Shutdown();
    ...
    inline bool AcquisitionMaster::WaitForConnectedNodes(size_t nodeCount, uint32_t timeoutMs)
    {
        return service_->WaitForConnectedNodes(nodeCount, timeoutMs);
    }

    inline size_t AcquisitionMaster::ConnectedNodeCount() const
    {
        return service_->ConnectedNodeCount();
    }

    inline bool AcquisitionMaster::SendConfigureToNode(const std::string &nodeId, const AcquisitionTask &task, const std::string &message)
    {
        try
        {
    ...
    inline size_t AcquisitionMaster::BroadcastConfigure(const AcquisitionTask &task, const std::string &message)
    {
        try
        {
            const auto effectiveTask = BuildEffectiveGlobalTask(task);
    ...
    inline void AcquisitionMaster::DistributeTasks()
    {
    ...
    inline size_t AcquisitionMaster::SendStart(uint64_t plannedStartTimeMs, uint32_t durationMs)
    {
        return service_->BroadcastCommand(MakeStartCommand(plannedStartTimeMs, durationMs));
    }

    inline size_t AcquisitionMaster::SendStop(const std::string &reason)
    {
        return service_->BroadcastCommand(MakeStopCommand(reason));
    }

    inline size_t AcquisitionMaster::SendShutdown(const std::string &reason)
    {
        return service_->BroadcastCommand(MakeShutdownCommand(reason));
    }

    inline std::vector<AcquisitionControlServiceImpl::ConnectedNode> AcquisitionMaster::SnapshotNodes() const
    {
